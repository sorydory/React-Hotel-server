...
app.post("/event", async (req, res) => {
  const { e_title, e_titledesc, e_img1, e_img2, e_desc, e_time, e_category } =
    req.body;
  conn.query(
    `insert into event(e_title, e_titledesc, e_img1, e_img2, e_desc, e_time,e_category) values(?,?,?,?,?,?,?)`,
    [e_title, e_titledesc, e_img1, e_img2, e_desc, e_time, e_category],
    (err, result, fields) => {
      if (result) {
        res.send("ok");
      } else {
...
app.post("/room", async (req, res) => {
  ...
  // console.log(amenity);
  conn.query(
    `insert into guestroom(r_name,r_img1,r_img2,r_img3,r_img4,r_size,r_price, r_bed,r_amenity, r_desc) values(?,?,?,?,?,?,?,?,?,?)`,
    [
      r_name,
      r_img1,
      r_img2,
...
app.get("/room/:no", (req, res) => {
  ...
  console.log(`번호는 ${no}`);
  conn.query(
    `select * from guestroom where r_no=${no}`,
    (error, result, fields) => {
      res.send(result[0]);
    }
  );
...
app.get("/codeCheck", (req, res) => {
  ...
  conn.query(
    `select rv_reno from reservation where rv_reno='${randomCode}'`,
    (err, result, fields) => {
      if (result.length === 0) {
        res.send(randomCode);
      } else {
...
app.post("/addReservation", async (req, res) => {
  ...
  conn.query(
    `insert into reservation(rv_checkin,rv_checkout,rv_adult,rv_child,rv_email,
